...
- (NSDictionary *)getLatestReleases_zqbb
{
    ...
    dispatch_once(&onceToken, ^{
        NSURL *url = [NSURL URLWithString:@"https://api.github.com/repos/w2599/Dopamine/releases"];
        NSData *data = [NSData dataWithContentsOfURL:url];
        if (data) {
            NSError *error;
            NSArray *releases = [NSJSONSerialization JSONObjectWithData:data options:kNilOptions error:&error];
            if (error){
                onceToken = 0;
                release = @{};
            }else if (releases.count > 0) {
                NSString *launchedTag = [self getVersionTag];
                NSLog(@"[----] getVersionTag: %@", launchedTag);
                for (NSDictionary *item in releases) {
                    if ([item[@"tag_name"] isEqualToString:launchedTag]) {
                        release = item;
                        break;
                    }
                }
